...
describe("P12: Seguridad & Bloqueo Biométrico Web", () => {
  ...
  it("enables biometric lock and updates localStorage", async () => {
    ...
    let success = false;
    await act(async () => {
      success = await result.current.setBiometricLockEnabled(true, "test@dom.app");
    });

    expect(success).toBe(true);
    expect(result.current.isBiometricLockEnabled).toBe(true);
    expect(localStorage.getItem("dom-biometric-enabled")).toBe("true");
  });
  ...
  it("locks and unlocks the application with biometrics", async () => {
    ...
    await act(async () => {
      await result.current.setBiometricLockEnabled(true, "test@dom.app");
    });
  ...
  it("updates and persists biometric timeout minutes", async () => {
    ...
    expect(localStorage.getItem("dom-biometric-timeout")).toBe("5");
  });
});
